...
package ir.telgeram.ui.ActionBar;
...
public class ActionBar extends FrameLayout {
    ...
    private static ArrayList<View> listofall = new ArrayList<>(); // Adel
    private ImageView ghostView; // Adel
    ...
    public ActionBar(Context context) {
        super(context);
        listofall.add(this); // Adel
        ThemeChanger.actionbar=this; // Adel
        createGhostModeImage(); // Adel
        changeGhostModeVisibility(); // Adel
    }
    ...
    private void createSubtitleTextView() {
        ...
        subtitleTextView.setTypeface(AndroidUtilities.getTypeface("fonts/rmedium.ttf")); // Adel
    ...
    public void createGhostModeImage() {
        ghostView = new ImageView(getContext());
        ghostView.setImageResource(R.mipmap.ic_ghost_telegram);
        initThemeGhostIcon();
        ghostView.setPadding(20, 20, 20, 20);
        addView(ghostView, -2, LayoutHelper.createFrame(-2, -2, 17));
    }

    // Adel
    private void initThemeGhostIcon() {
        if ( getContext() != null) {
            int a = ThemeChanger.ColorFilter(ThemeChanger.getcurrent().getActionbarcolor(), 21);
            if (this.ghostView != null) {
                this.ghostView.setColorFilter(a, PorterDuff.Mode.SRC_IN);
            }
        }
    }
    ...
    public void changeGhostModeVisibility() {
        if (Setting.getGhostMode()) {
            this.ghostView.setVisibility(View.VISIBLE);
            initThemeGhostIcon();
            return;
        }
        this.ghostView.setVisibility(View.GONE);
    }
