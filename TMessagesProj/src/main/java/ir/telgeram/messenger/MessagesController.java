...
import ir.telgeram.messenger.voip.VoIPService;
...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate
{
	...
	public                  ArrayList<TLRPC.TL_dialog>  dialogsChannelOnly              = new ArrayList<>(); // Adel
	public                  ArrayList<TLRPC.TL_dialog>  dialogsJustGroupsOnly           = new ArrayList<>(); // Adel
	public                  ArrayList<TLRPC.TL_dialog>  dialogsContactOnly              = new ArrayList<>(); // Adel
	...
	public                  ArrayList<TLRPC.TL_dialog>  dialogsUnreadOnly               = new ArrayList<>();
	...
	public void cleanup()
	{
		...
		fullUsers.clear();
		dialogs.clear();
		dialogsChannelOnly.clear(); // Adel
		dialogsJustGroupsOnly.clear(); // Adel
		dialogsContactOnly.clear(); // Adel
		dialogsFavoriteOnly.clear(); // Adel
		dialogsBotOnly.clear(); // Adel
		...
		if (first)
		{
			...
			if (dialog != null)
			{
				...
				if (onlyHistory == 0 || onlyHistory == 3)
				{
					...
					dialogsChannelOnly.remove(dialog); // Adel
					dialogsJustGroupsOnly.remove(dialog); // Adel
					dialogsContactOnly.remove(dialog); // Adel
					dialogsFavoriteOnly.remove(dialog); // Adel
					dialogsBotOnly.remove(dialog); // Adel
					dialogsUnreadOnly.remove(dialog); // Adel
	...
	private void deleteADialog(TLRPC.TL_dialog dialog) // Adel
	{
		dialogsChannelOnly.remove(dialog); // Adel
		dialogsJustGroupsOnly.remove(dialog); // Adel
		dialogsContactOnly.remove(dialog); // Adel
			...
			if (ConnectionsManager.getInstance().getPauseTime() == 0 && ApplicationLoader.isScreenOn && !ApplicationLoader.mainInterfacePausedStageQueue)
			{
				if (ApplicationLoader.mainInterfacePausedStageQueueTime != 0 && Math.abs(ApplicationLoader.mainInterfacePausedStageQueueTime - System.currentTimeMillis()) > 1000)
				{
					if (statusSettingState != 1 && (lastStatusUpdateTime == 0 || Math.abs(System.currentTimeMillis() - lastStatusUpdateTime) >= 55000 || offlineSent))
					{
						...
						req.offline = Setting.getGhostMode(); // Adel
	...
	{
		Log.d("_Adel", "MsgContoller sortDialogs");
		...
		for (int a = 0; a < dialogs.size(); a++)
		{
			...
			if (lower_id != 0 && high_id != 1)
			{
				...
				if (DialogObject.isChannel(d))
				{
					...
					if (chat != null && chat.megagroup) // Adel, this is super group
					{
						dialogsGroupsOnly.add(d); // Adel
					}
					if (chat != null && !chat.megagroup) // Adel
					{
						dialogsChannelOnly.add(d); // Adel
					}
				}
				else if (lower_id < 0)
				{
					...
					dialogsGroupsOnly.add(d);
					dialogsJustGroupsOnly.add(d); // Adel
				}
				else if (lower_id > 0) // Adel
				{
					TLRPC.User user = getUser(Integer.valueOf((int) d.id));
					if (user == null || !user.bot)
					{
						dialogsContactOnly.add(d);
					}
					else
					{
						dialogsBotOnly.add(d);
					}
				}
			}
			// Adel
			if (FavoriteController.IsFaver(Long.valueOf(d.id)))
			{
				dialogsFavoriteOnly.add(d);
			}

			// Adel
			if (d.unread_count > 0)
			{
				dialogsUnreadOnly.add(d);
			}
		}

		// Adel
		DialogsAdapter.getInstance().notifyDataSetChanged();
